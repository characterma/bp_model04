...
class Greeter(anbp_pb2_grpc.GreeterServicer):
    def ANBP(self, request, context):
        ...
        try:
            ...
            #############################################
            # nn model的预测
            df_nn_data = pd.DataFrame(data=ppg_values, columns=feature_names_sql)
            # 增加 'Age','Gender','Height','Weight'
            df_nn_data['Age'] = userInfo['age']
            df_nn_data['Gender'] = userInfo['gender']
            df_nn_data['Height'] = userInfo['height']
            df_nn_data['Weight'] = userInfo['weight']

            df_nn_data = df_nn_data[feature_names_nn]

            nn_model = Deeplearningmodel()
            result = nn_model.model_predict(df_nn_data)
            print("nn models results: {}".format(result))
