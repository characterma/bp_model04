...
for sdr in tqdm(set_date_range):
    ...
    for a in range(temp_dft.shape[0]): 
        ...
        # ppg_values = temp_dft.iloc[a]['ppg_values']
        ...
        for ps in ppg_values_all.split(";"):
            ...
            # 去除AC为 0 的值
            temp_result = temp_result[temp_result['AC'] > 0.0]
            temp_result = temp_result[temp_result['A1'] > 0.0]
            temp_result = temp_result[temp_result['A2'] > 0.0]
            temp_result.dropna(axis=0, how="any", inplace=True)
            ...
            temp_result['date'] = temp_dft.iloc[a]['DATE']
            # temp_result['ppg_time'] = temp_dft.iloc[a]['ppg_time']
            temp_result['ppg_time'] = ppg_time
            # 拼接到 df_results
            df_test = pd.concat([df_test, temp_result], ignore_index=True)


weight_old_model = 0.0
weight_kefu_model = 0.0
weight_personal_model = 1.0
...
df_test.to_csv("predict_results/{}_pred_weights02_date_range.csv".format(wear_user_id), index=False)
...
# 遍历 每个 ppg_value数据
# for sdr in tqdm(set_date_range):
#     plot_sbp_dbp_by_ppg_time(df_test[df_test['date']==sdr][['ppg_time', 'weight_sbp', 'weight_dbp']])